...
\item \textbf{\code{fp\_*} perturbations keep the gold relation's \code{relation\_type}.} Only \code{entity\_B} is replaced. We deliberately do \emph{not} randomize the relation type for fp samples, because that would overlap with \code{label\_flip} (the model could then detect fp via the wrong-relation signal rather than via the entity swap). Keeping the gold relation type isolates the wrong-\code{entity\_B} signal cleanly, so per-type F1 actually measures what we claim.
...
\noindent
The top 3 cover about 96--97\,\% of every split. \code{Drug\_Interaction} and \code{Conversion} have \textbf{zero examples in dev}, which means there is no evaluation signal for them no matter how the filter is trained. \code{Bind}, \code{Cotreatment}, \code{Comparison} are borderline at single-digit dev counts.

\medskip
\noindent
We therefore build the dataset in two variants:

\begin{itemize}
\item \textbf{reduced (default):} keep only \code{Association}, \code{Positive\_Correlation}, \code{Negative\_Correlation}. This is what the filter is actually trained and evaluated on.
\item \textbf{full:} every relation type included. Used as an ablation to confirm that adding the rare classes doesn't help (and that they aren't a stealthy source of label noise).
\end{itemize}
